...
with tqdm(total=total_rows, desc="Processing file: " + rs_file) as pbar:
    for i, rs_dp in enumerate(rs_df.iter_rows()):
        ...
        df2 = pl.DataFrame({'timestamp':[str(timestamp)], 'ss.x':[str(rs_dp[1])], 'ss.y':[str(rs_dp[2])], 'ss.z':[str(rs_dp[3])], 'rc.x':[str(rs_dp[4])], 'rc.y':[str(rs_dp[5])], 'rc.z':[str(rs_dp[6])], 'Dist':[str(dist)], 'Bend':[str(bend)]})
        with open(output_path + f'/{rs_file}', 'ab') as f:

...
bool Adafruit_MLX90393::begin_I2C(uint8_t i2c_addr, TwoWire *wire) {
  ...
  /* FIXME: setting fastmost at 400KHz TODO: SET A CONSTANT SOMEWHERE*/ 
...
bool Adafruit_MLX90393::exitMode(void) {
  ...
  return ((tx, sizeof(tx), NULL, 0, 0) == MLX90393_STATUS_OK);
}
...
bool Adafruit_MLX90393::readData(float *x, float *y, float *z) {
  ...
  /* TODO: THIS IS WHERE THE DELAYS OCCUR FOR THE FILTERING AND OSC */
  //Serial.println(mlx90393_tconv[_dig_filt][_osr] + 10);
  //delay(mlx90393_tconv[_dig_filt][_osr]);
  // mlx90393_tconv[_dig_filt][_osr];
  //delayMicroseconds(1300); /*edge of stability 1282*/
  return readMeasurement(x, y, z);
}

// BOTTLENECK (I get 630 HZ)
bool Adafruit_MLX90393::writeRegister(uint8_t reg, uint16_t data) {
  ...
  /* Perform the transaction. */ /* TODO: Bottleneck 125992 to 633Hz*/
  return (transceive(tx, sizeof(tx), NULL, 0, 0) == MLX90393_STATUS_OK);
}

// HUUUGE BOTTLENECK (125992Hz)
bool Adafruit_MLX90393::readRegister(uint8_t reg, uint16_t *data) {
  ...
  /* Perform the transaction. */ /* TODO: bottleneck 125992Hz -> 633Hz */
  if (transceive(tx, sizeof(tx), rx, sizeof(rx), 0) != MLX90393_STATUS_OK) {
    return false;
  }
